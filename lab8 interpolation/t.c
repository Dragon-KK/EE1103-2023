...
int main(int argc, char** argv){
    ...
    if (findCubicEquations(x, y, options.order + 1, cubics)){
        for (double xx = options.xstart; xx < options.xend; xx+=0.1){
            int cubicIndex = (xx - options.xstart) * options.order / (options.xend - options.xstart);
            ...
            double val = cubics[cubicIndex].a0 + cubics[cubicIndex].a1*xx + cubics[cubicIndex].a2*pow(xx, 2) + cubics[cubicIndex].a3*pow(xx, 3);
