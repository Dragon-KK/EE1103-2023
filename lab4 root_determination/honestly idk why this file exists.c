...
#define BISECTION_X_START -10.0f
#define BISECTION_X_END 10.0f
#define BISECTION_ERROR 0.001f // i.e 10%

#define FALSE_POSITION_X_START 0.5f
...
struct Polynomial{
    int order;
    float* coefficients; // Size will be of order+1
};

struct Polynomial poly = {-1};
...
int main(int argc, char const *argv[])
{
    ...
    if (argc > 2){
        ...
        if (order < 0){
            printf("Invalid order bro!\n");
            return 1;
        }
        if (argc - 3 < order + 1){
            printf("Please provide correct number of coefficients bro\n");
            return 1;
        }
        float* coeffs = malloc(sizeof(float) * (order + 1));
        ...
        //     printf("%fx^%d + ",poly.coefficients[i], poly.order-i);
        // }
        // printf("!\n");
    }
...
double function(double x){
    ...
    double ans = 0;
    for (int i = 0; i < poly.order+1; i++){
        ans += powl(x, poly.order-i) * poly.coefficients[i];
    }
    return ans;
